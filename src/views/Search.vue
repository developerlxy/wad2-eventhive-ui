...
export default {
      ...
      getAllSearchEvents: function(name, location, groupSize, startDate, endDate){
        ...
        for (let e of this.$store.state.events){
          ...
          if (location!=""){
            ...
            let distanceBetween = this.getDistanceFromLatLonInKm(this.location1_lat, this.location1_long, this.location2_lat, this.location2_long)
          ...
          if (eName.includes(name.toLowerCase()) && dateMatch && locationMatch && groupMatch){
            this.events.push(e)
          }
        }
      }
    ...
    '$route.params': {
        handler() {
            ...
            this.getAllSearchEvents(nameSearch, this.location1, groupSizeSearch, startDateSearch, endDateSearch)
